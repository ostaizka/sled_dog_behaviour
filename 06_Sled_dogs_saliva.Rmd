...
#  stat_compare_means(show.legend = FALSE) +
  theme(axis.text.x = element_text(vjust = 0.5, size = 14),
        ...
        axis.line = element_line(size = 0.5, linetype = "solid", colour = "black")) +
  guides(colour = guide_legend(override.aes = list(size = 6))) +
  labs(x = "Sled dog group", y = "Neutral alpha diversity")
 dev.off()
```
...
```
```{r}
load("resources/beta_div_saliva.Rdata")
...
#### Species profile
```{r species_profile_S}
species_profile <- div_profile(saliva.table, qvalues=seq(from = 0, to = 3, by = (0.2)),hierarchy=hierarchy.saliva[,c(1,4)])
...
##### Richness
```{r rich_S}
divq0 <- div_test(saliva.table,qvalue=0,hierarchy=hierarchy.saliva[,c(1,4)])
divq0_value <- divq0$data
q0 <- merge(divq0_value, hierarchy.saliva, by="Sample")
means_q0_byWorking_status <- q0 %>% 
  group_by(Working_status) %>% 
  summarise_at(.vars = names(.)[3],.funs = c(mean="mean", sd="sd"))
as.data.frame(means_q0_byWorking_status)
```

###### Negative Binomial
```{r NB_S}
Modelq0GLMMNB <- glmer.nb(Value ~ Group + (1|Pack), data = q0)
summary(Modelq0GLMMNB)
r.squaredGLMM(Modelq0GLMMNB)
```
###### Plot the model
```{r plotq0_S}
plot_model(Modelq0GLMMNB,type="pred",terms = "Group", show.data = TRUE,jitter=0.2, connect.lines = TRUE)+theme_bw()
```
#### Highlight the leaders
```{r highlight_Leader_F, echo=FALSE, fig.cap="Diversity at q=0. Note: leaders in red"}
highlight_df <- q0 %>% 
             filter(Working_status=="Lead")
q0 %>% 
  ggplot(aes(x=Pack,y=Value)) +
  geom_boxplot() + 
...
```{r neutral_S}
divq1 <- div_test(saliva.table,qvalue=1,hierarchy=hierarchy.saliva[,c(1,4)])
divq1_value <- divq1$data
q1 <- merge(divq1_value, hierarchy.saliva, by="Sample")
means_q1_byWorking_statusPack <- q1 %>% 
  group_by(Pack,Working_status) %>% 
  summarise_at(.vars = names(.)[3],.funs = c(Maximum="max", Minimum="min",mean="mean", sd="sd"))
means_q1_byWorking_status <- q1 %>% 
  group_by(Working_status) %>% 
  summarise_at(.vars = names(.)[3],.funs = c(mean="mean", sd="sd"))
as.data.frame(means_q1_byWorking_statusPack)
as.data.frame(means_q1_byWorking_status)
```

###### Negative Binomial
```{r lme_S}
Modelq1 <- lme(fixed = Value ~ Group, data = q1,
               random = ~ 1 | Pack)
summary(Modelq1)
```
###### Plot the model
```{r plotq1_S}
plot_model(Modelq1,type="pred",terms = "Group", show.data = TRUE,jitter=0.2, connect.lines = TRUE)+theme_bw()
```
#### Highlight the leader
```{r highlight_Leader_q1_F, echo=FALSE, fig.cap="Diversity at q=1. Note: leaders in red"}
highlight_df <- q1 %>% 
             filter(Working_status=="Lead")
q1 %>% 
  ggplot(aes(x=Pack,y=Value)) +
...
```{r phylo_S}
divq1P <- div_test(saliva.table,qvalue=1,hierarchy=hierarchy.saliva[,c(1,4)],tree=tree_saliva)
```

```{r phylo_model_S}
divq1P_value <- divq1P$data
q1P <- merge(divq1P_value, hierarchy.saliva, by="Sample")
means_q1P_byWorking_statusPack <- q1P %>% 
  group_by(Pack,Working_status) %>% 
  summarise_at(.vars = names(.)[3],.funs = c(Maximum="max", Minimum="min",mean="mean", sd="sd"))
means_q1P_byWorking_status <- q1P %>% 
  group_by(Working_status) %>% 
  summarise_at(.vars = names(.)[3],.funs = c(mean="mean", sd="sd"))
as.data.frame(means_q1P_byWorking_statusPack)
as.data.frame(means_q1P_byWorking_status)
Modelq1P <- lme(fixed = Value ~ Group, data = q1P,
                random = ~ 1 | Pack)
summary(Modelq1P)
...
###### Plot the model
```{r plotq1P_S}
plot_model(Modelq1P,type="pred",terms = "Group", show.data = TRUE,jitter=0.2, connect.lines = TRUE)+theme_bw()
```
...
```{r read_pair_dis_S}
pairdis.q0 <- readRDS(paste(projectdirR,"Data/pairdis.q0.",codeSaliva,".RData",sep=""))
pairdis.q1 <- readRDS(paste(projectdirR,"Data/pairdis.q1.",codeSaliva,".RData",sep=""))
pairdis.q0P <- readRDS(paste(projectdirR,"Data/pairdis.q0P.",codeSaliva,".RData",sep=""))
pairdis.q1P <- readRDS(paste(projectdirR,"Data/pairdis.q1P.",codeSaliva,".RData",sep=""))
```

```{r pair_dis_S}
#pairdis.q0 <- pair_dis(saliva.table,qvalue=0,hierarchy=hierarchy.saliva[,c(1,4)])
#saveRDS(pairdis.q0,paste(projectdirR,"Data/pairdis.q0.",codeSaliva,".RData",sep=""))
#pairdis.q1 <- pair_dis(saliva.table,qvalue=1,hierarchy=hierarchy.saliva[,c(1,4)])
#saveRDS(pairdis.q1,paste(projectdirR,"Data/pairdis.q1.",codeSaliva,".RData",sep=""))
#pairdis.q0P <- pair_dis(saliva.table,qvalue=0,hierarchy=hierarchy.saliva[,c(1,4)],tree=tree_saliva)
#saveRDS(pairdis.q0P,paste(projectdirR,"Data/pairdis.q0P.",codeSaliva,".RData",sep=""))
#pairdis.q1P <- pair_dis(saliva.table,qvalue=1,hierarchy=hierarchy.saliva[,c(1,4)],tree=tree_saliva)
#saveRDS(pairdis.q1P,paste(projectdirR,"Data/pairdis.q1P.",codeSaliva,".RData",sep=""))
```
...
u0Pn <- pairdis.q0P$L1_UqN
u0Pn.dist <- as.dist(u0Pn)
u1Pn <- pairdis.q1P$L1_UqN
u1Pn.dist <- as.dist(u1Pn) 
```

##### Permanova
```{r permanova_strata_S}
adonis2(u0n.dist ~ sample_data(physeq_saliva)$Working_status, strata = sample_data(physeq_saliva)$Pack, permutations = 999)
adonis2(u1n.dist ~ sample_data(physeq_saliva)$Working_status, strata = sample_data(physeq_saliva)$Pack, permutations = 999)
adonis2(u0Pn.dist ~ sample_data(physeq_saliva)$Working_status, strata = sample_data(physeq_saliva)$Pack, permutations = 999)
adonis2(u1Pn.dist ~ sample_data(physeq_saliva)$Working_status, strata = sample_data(physeq_saliva)$Pack, permutations = 999)
```

##### Permanova
```{r permanovaAll_S}
adonis2(u0n.dist ~ sample_data(physeq_saliva)$Working_status + 
         sample_data(physeq_saliva)$Pack + sample_data(physeq_saliva)$Location + 
         sample_data(physeq_saliva)$Sex + sample_data(physeq_saliva)$Age, permutations = 999)
adonis2(u1n.dist ~ sample_data(physeq_saliva)$Working_status + 
         sample_data(physeq_saliva)$Pack + sample_data(physeq_saliva)$Location + 
          sample_data(physeq_saliva)$Sex + sample_data(physeq_saliva)$Age, permutations = 999)
adonis2(u0Pn.dist ~ sample_data(physeq_saliva)$Working_status + sample_data(physeq_saliva)$Pack + 
         sample_data(physeq_saliva)$Location + sample_data(physeq_saliva)$Sex + 
         sample_data(physeq_saliva)$Age, permutations = 999)
adonis2(u1Pn.dist ~ sample_data(physeq_saliva)$Working_status + sample_data(physeq_saliva)$Pack + 
         sample_data(physeq_saliva)$Location + sample_data(physeq_saliva)$Sex + 
         sample_data(physeq_saliva)$Age, permutations = 999)
```
...
##### Calculate NMDS
```{r NMDS_S}
nmds <- metaMDS(as.dist(pairdis.q1$L1_UqN),k=2,trymax=100)
NMDS=data.frame(x=nmds$point[,1],y=nmds$point[,2])
NMDS <- merge(NMDS,saliva.metatable,by="row.names")
head(NMDS)
```

##### Centroids
```{r centroids_S}
NMDS.centroids=aggregate(NMDS[,c(2:3)],by=list(NMDS$Pack),FUN=mean)
colnames(NMDS.centroids) <- c("Pack","x_cen","y_cen")
NMDS=merge(NMDS,NMDS.centroids,by="Pack")
```

##### Plots
```{r plots_nmds_S}
ggplot(NMDS, aes(x=x,y=y,colour=Pack,shape=Working_status)) +
  geom_point(size=2) +
...
##### Calculate NMDS by Working status
```{r NMDS_S_by_WS}
nmds <- metaMDS(as.dist(pairdis.q1$L1_UqN),k=2,trymax=100)
NMDS=data.frame(x=nmds$point[,1],y=nmds$point[,2])
NMDS <- merge(NMDS,saliva.metatable,by="row.names")
...
##### Centroids by Working status
```{r centroids_S_by_WS}
NMDS.centroids=aggregate(NMDS[,c(2:3)],by=list(NMDS$Working_status),FUN=mean)
colnames(NMDS.centroids) <- c("Working_status","x_cen","y_cen")
NMDS=merge(NMDS,NMDS.centroids,by="Working_status")
```

##### Plots by Working status
```{r plots_nmds_S_by_WS}
ggplot(NMDS, aes(x=x,y=y,colour=Working_status, shape=Pack)) +
  geom_point(size=2) +
  geom_point(aes(x=x_cen,y=y_cen),alpha=0) +
  scale_color_brewer(palette="Dark2") +
  scale_shape_manual(values=1:nlevels(NMDS$Pack)) +
  geom_segment(aes(x=x_cen, y=y_cen, xend=x, yend=y), alpha=0.3) +
  theme_light() +
  theme(axis.line = element_line(colour = "grey"),
        panel.grid.major = element_blank(),
